...
function optimize(requestedQty: number, pkgUnit: number, stock: number, tiers: PriceTier[]) {
  ...
  const candidates = new Set([requestedQty, pkgQty]);
  ...
  const pool = (scored.filter(c => c.feasible).length ? scored.filter(c => c.feasible) : scored)
...
// Distributori da includere nei risultati (aggiungi/rimuovi liberamente)
...
// Link affiliati — aggiungi i tuoi codici qui
const AFFILIATE_PARAMS: Record<string, string> = {
  "Mouser":       "",   // es. "?ref=YOUR_CODE"
  "Digi-Key":     "",
  "Farnell":      "",
  "TME":          "",
  "RS Components":"",
...
async function searchNexar(mpn: string, qty: number): Promise<any[]> {
  ...
  const res = await fetch(NEXAR_API_URL, {
    ...
    body:    JSON.stringify({ query, variables: { mpn, qty } }),
  });
  if (!res.ok) return [];
  const json = await res.json();

  const hits  = json?.data?.supSearchMpn?.hits ?? [];
  const parts: any[] = [];
  ...
  for (const hit of hits) {
    ...
    for (const seller of sellers) {
      ...
      // Filtra solo i distributori che vogliamo
      ...
      for (const offer of (seller?.offers ?? [])) {
        ...
        // Costruisci URL con affiliazione
        const baseUrl    = offer?.clickUrl ?? `https://octopart.com/search?q=${encodeURIComponent(mpn)}`;
...
// ─────────────────────────────────────────────────────────────────────────────
//  AGGIUNGI ALTRI DISTRIBUTORI QUI SE NECESSARIO
//  Esempio: async function searchTME(mpn, qty) { ... }
//  Poi aggiungilo in SOURCES sotto
// ─────────────────────────────────────────────────────────────────────────────

const SOURCES = [
  searchNexar,
  // searchTME,   // ← aggiungi qui
];
  ...
  try {
    ...
    const results: ResultRow[] = await Promise.all(
      bom.map(async ({ mpn, qty }): Promise<ResultRow> => {
        ...
        // Chiama tutte le sorgenti in parallelo
